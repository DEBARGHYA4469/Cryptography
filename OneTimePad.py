#Code to attack a many-time-pad construction and decrypt all its contenrs
import re
import sys
from operator import methodcaller

#the cipher texts are hex-encoded..
c1 = ""
c2 = "234c02ecbbfbafa3ed18510abd11fa724fcda2018a1a8342cf064bbde548b12b07df44ba7191d9606ef4081ffde5ad46a5069d9f7f543bedb9c861bf29c7e205132eda9382b0bc2c5c4b45f919cf3a9f1cb74151f6d551f4480c82b2cb24cc5b028aa76eb7b4ab24171ab3cdadb8356f"
c3 = "32510ba9a7b2bba9b8005d43a304b5714cc0bb0c8a34884dd91304b8ad40b62b07df44ba6e9d8a2368e51d04e0e7b207b70b9b8261112bacb6c866a232dfe257527dc29398f5f3251a0d47e503c66e935de81230b59b7afb5f41afa8d661cb"
c4 = ""
c5 = "3f561ba9adb4b6ebec54424ba317b564418fac0dd35f8c08d31a1fe9e24fe56808c213f17c81d9607cee021dafe1e001b21ade877a5e68bea88d61b93ac5ee0d562e8e9582f5ef375f0a4ae20ed86e935de81230b59b73fb4302cd95d770c65b40aaa065f2a5e33a5a0bb5dcaba43722130f042f8ec85b7c2070"
c6 = ""
c7 = ""
c8 = "315c4eeaa8b5f8bffd11155ea506b56041c6a00c8a08854dd21a4bbde54ce56801d943ba708b8a3574f40c00fff9e00fa1439fd0654327a3bfc860b92f89ee04132ecb9298f5fd2d5e4b45e40ecc3b9d59e9417df7c95bba410e9aa2ca24c5474da2f276baa3ac325918b2daada43d6712150441c2e04f6565517f317da9d3"
c9 = ""
c10 ="466d06ece998b7a2fb1d464fed2ced7641ddaa3cc31c9941cf110abbf409ed39598005b3399ccfafb61d0315fca0a314be138a9f32503bedac8067f03adbf3575c3b8edc9ba7f537530541ab0f9f3cd04ff50d66f1d559ba520e89a2cb2a83"
c11 ="32510ba9babebbbefd001547a810e67149caee11d945cd7fc81a05e9f85aac650e9052ba6a8cd8257bf14d13e6f0a803b54fde9e77472dbff89d71b57bddef121336cb85ccb8f3315f4b52e301d16e9f52f904"


def main():
	
	#create a list of the cipher texts
	cipher_text_encoded = []
	cipher_text_encoded.append(c1)
	cipher_text_encoded.append(c2)
	cipher_text_encoded.append(c3)
	cipher_text_encoded.append(c4)
	cipher_text_encoded.append(c5)
	cipher_text_encoded.append(c6)
	cipher_text_encoded.append(c7)
	cipher_text_encoded.append(c8)
	cipher_text_encoded.append(c9)
	cipher_text_encoded.append(c10)
	cipher_text_encoded.append(c11)

	Ciphers  = map(methodcaller("decode","hex"),cipher_text_encoded)

	#check ="We can factor the number 15 with quantum computers. We can also factor the number 15 with a dog trained to bark three times - Robert Harley"
	#print(strxor(strxor(Ciphers[0],Ciphers[1]),check))

	#Crib dragging technique

    
	while(1):
		print("pick a crib")
		crib = raw_input()
		print "pick ciphers to try"
		xx = input()
		yy = input()

		xxyy = strxor(Ciphers[xx],Ciphers[yy])
		length = len(xxyy)
		for i in range(length):
			temp = xxyy[i:]
			drag = strxor(temp,crib)
			if  isreadable(drag):
				break_the_cipher(i,drag,length,'.')

		print "\n\nGuess key (when the crib starts lining up):"
		key = strxor(Ciphers[xx], crib)
		print "key: %s"%key.encode('hex')

		print "\n\nDecrypt all messages using the key:"
		for j, cypher in enumerate(Ciphers) :
			print "[%d] %s" % (j, strxor(cypher, key))






def isreadable(str): #check readability 
	return bool(re.search('^[a-zA-Z0-9\., \'\"\-_\:\(\)]+$', str))

def break_the_cipher(i, code, maxLen, filler): 
	prefix =  filler*i
	ln = len(code)
	postfix = filler*(maxLen-i-ln)
	print("%s%s%s" % (prefix,code,postfix))

def strxor(str1,str2):
	if len(str1) > len(str2):
		return "".join([chr(ord(x) ^ ord(y)) for (x,y) in zip(str1[:len(str2)],str2) ])
	else : 
		return "".join([chr(ord(x) ^ ord(y)) for (x,y) in zip(str1,str2[:len(str1)]) ])


main()